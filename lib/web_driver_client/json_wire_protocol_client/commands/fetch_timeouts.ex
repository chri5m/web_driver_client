defmodule WebDriverClient.JSONWireProtocolClient.Commands.FetchTimeouts do
  @moduledoc false
  ...
  @spec parse_response(HTTPResponse.t()) ::
          {:ok, map()} | {:error, UnexpectedResponseError.t() | WebDriverError.t()}
  def parse_response(%HTTPResponse{} = http_response) do
    with {:ok, jwp_response} <- ResponseParser.parse_response(http_response),
         ...
         {:ok, timeouts} <- ResponseParser.parse_value(jwp_response) do
      {:ok, timeouts}
    end
  end
end
