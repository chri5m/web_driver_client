defmodule WebDriverClient.W3CWireProtocolClient.Commands.MaximizeWindow do
  @moduledoc false
  ...
  def send_request(%Session{id: id, config: %Config{} = config}) do
    ...
    url = "/session/#{id}/window/maximize"
  ...
  def parse_response(%HTTPResponse{} = http_response) do
    with {:ok, w3c_response} <- ResponseParser.parse_response(http_response),
         ...
         {:ok, size} <- ResponseParser.parse_rect(w3c_response) do
      {:ok, size}
    end
  end
end
