defmodule WebDriverClientTest do
  ...
  test "set_timeouts/2 with jwp session returns :ok on success", %{
    ...
    session = TestData.session(config: constant(config)) |> pick()
    resp = JWPTestResponses.set_timeouts_response() |> pick()
    timeouts = %{implicit: 400, pageLoad: 5000, script: 2000}

    stub_bypass_response(bypass, resp)

    assert :ok = WebDriverClient.set_timeouts(session, timeouts)
  end

  @tag protocol: :w3c
  test "set_timeouts/2 with w3c session returns :ok on success", %{
    config: config,
    bypass: bypass
  } do
    session = TestData.session(config: constant(config)) |> pick()
    resp = W3CTestResponses.set_timeouts_response() |> pick()
    timeouts = %{implicit: 400, pageLoad: 5000, script: 2000}
  ...
  for protocol <- @protocols do
    @tag protocol: protocol
    test "set_timeouts/2 with #{protocol} session returns appropriate errors on various server responses",
         %{config: config, bypass: bypass, protocol: protocol} do
      scenario_server = set_up_error_scenario_tests(protocol, bypass)
      timeouts = %{implicit: 400, pageLoad: 5000, script: 2000}
      ...
      for error_scenario <- basic_error_scenarios(protocol) do
        ...
        assert_expected_response(
          protocol,
          WebDriverClient.set_timeouts(session, timeouts),
          error_scenario
        )
      end
    end
  ...
  @tag protocol: :w3c
  test "execute_script/3 with w3c session returns :ok on success", %{
    config: config,
    bypass: bypass
  } do
    session = TestData.session(config: constant(config)) |> pick()
    resp = W3CTestResponses.execute_script_response() |> pick()
    stub_bypass_response(bypass, resp)

    script = "return 1"

    assert {:ok, _result} = WebDriverClient.execute_script(session, script)
  end

  @tag protocol: :jwp
  test "execute_script/3 with JWP session returns :ok on success", %{
    config: config,
    bypass: bypass
  } do
    session = TestData.session(config: constant(config)) |> pick()
    resp = JWPTestResponses.execute_script_response() |> pick()
    stub_bypass_response(bypass, resp)
  ...
  for protocol <- @protocols do
    @tag protocol: protocol
    test "execute_script/3 with #{protocol} session returns appropriate errors on various server responses",
         %{config: config, bypass: bypass, protocol: protocol} do
      scenario_server = set_up_error_scenario_tests(protocol, bypass)
      ...
      for error_scenario <- basic_error_scenarios(protocol) do
        ...
        assert_expected_response(
          protocol,
          WebDriverClient.execute_script(session, script),
          error_scenario
        )
      end
    end
  ...
  @tag protocol: :w3c
  test "switch_to_parent_frame/1 with w3c session returns :ok on success", %{
    config: config,
    bypass: bypass
  } do
    session = TestData.session(config: constant(config)) |> pick()
    resp = W3CTestResponses.switch_to_parent_frame_response() |> pick()
    stub_bypass_response(bypass, resp)

    assert :ok = WebDriverClient.switch_to_parent_frame(session)
  end

  @tag protocol: :jwp
  test "switch_to_parent_frame/1 with JWP session returns :ok on success", %{
    config: config,
    bypass: bypass
  } do
    session = TestData.session(config: constant(config)) |> pick()
    resp = JWPTestResponses.switch_to_parent_frame_response() |> pick()
    stub_bypass_response(bypass, resp)
  ...
  for protocol <- @protocols do
    @tag protocol: protocol
    test "switch_to_parent_frame/1 with #{protocol} session returns appropriate errors on various server responses",
         %{config: config, bypass: bypass, protocol: protocol} do
      scenario_server = set_up_error_scenario_tests(protocol, bypass)
      ...
      for error_scenario <- basic_error_scenarios(protocol) do
        ...
        assert_expected_response(
          protocol,
          WebDriverClient.switch_to_parent_frame(session),
          error_scenario
        )
      end
    end
  ...
  @tag protocol: :w3c
  test "switch_to_default_content/1 with w3c session returns :ok on success", %{
    config: config,
    bypass: bypass
  } do
    session = TestData.session(config: constant(config)) |> pick()
    resp = W3CTestResponses.switch_to_default_content_response() |> pick()
    stub_bypass_response(bypass, resp)

    assert :ok = WebDriverClient.switch_to_default_content(session)
  end

  @tag protocol: :jwp
  test "switch_to_default_content/1 with JWP session returns :ok on success", %{
    config: config,
    bypass: bypass
  } do
    session = TestData.session(config: constant(config)) |> pick()
    resp = JWPTestResponses.switch_to_default_content_response() |> pick()
    stub_bypass_response(bypass, resp)
  ...
  for protocol <- @protocols do
    @tag protocol: protocol
    test "switch_to_default_content/1 with #{protocol} session returns appropriate errors on various server responses",
         %{config: config, bypass: bypass, protocol: protocol} do
      scenario_server = set_up_error_scenario_tests(protocol, bypass)
      ...
      for error_scenario <- basic_error_scenarios(protocol) do
        ...
        assert_expected_response(
          protocol,
          WebDriverClient.switch_to_default_content(session),
          error_scenario
        )
      end
    end
